...
function getStudentGradeGender(fullName) {
  ...
  const data = sheet.getDataRange().getValues();
  const key = normalizeName(fullName); // name from project sheet
  ...
  for (let r = 1; r < data.length; r++) { // skip header
    ...
    const lastFirst = normalizeName(`${last} ${first}`);
    const firstLast = normalizeName(`${first} ${last}`);

    if (key === lastFirst || key === firstLast) {
