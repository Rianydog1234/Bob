...
// replace gradeSortKey inside writeBalancedGroups (or replace the whole function if you prefer)
function gradeSortKey(grade) {
  if (!grade) return Number.MAX_SAFE_INTEGER;
  grade = grade.toString().trim().toUpperCase(); // <-- normalize to upper-case
  if (GRADE_ORDER && GRADE_ORDER[grade] !== undefined) return GRADE_ORDER[grade] * 1000;
  const m = grade.match(/^(\d+)\s*([A-Z])?$/i);
  if (m) {
