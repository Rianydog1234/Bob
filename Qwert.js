...
function getStudentGradeGender(studentName) {
  ...
  const sheet = ss.getSheets().find(s => s.getSheetId() === SOURCE_INFO_SHEET_GID);
  if (!sheet) throw new Error("Info sheet not found");
  const names = sheet.getRange(2, 1, sheet.getLastRow()-1, 2).getValues(); // only first & last
  const grades = sheet.getRange(2, 3, sheet.getLastRow()-1).getValues(); 

  // Normalize: remove extra spaces, trim
  const lookup = studentName.trim();

  // Try all combinations: Last First or First Last
  let matchIndex = names.findIndex(r => 
    (r[0].trim() + " " + r[1].trim() === lookup) || 
    (r[1].trim() + " " + r[0].trim() === lookup)
  );

  if (matchIndex === -1) throw new Error("Student info not found: " + studentName);

  return {
    grade: grades[matchIndex][0],
    gender: names[matchIndex][1].trim()
  };
}
  ...
  Object.entries(map).forEach(([project, col]) => {
    ...
    groups[project] = values
      ...
      .map((r, index) => {
        ...
        try {
          const info = getStudentGradeGender(rawNameCell, project, startRow + index);
          return {
            name: rawNameCell,
          ...
          // Log full detail but continue mapping
          Logger.log("ERROR in project '%s', row %s: %s", project, startRow + index, e.message);
          throw e; // still throw to stop the close if you want precise info
        }
      });
  });
